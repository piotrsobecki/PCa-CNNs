...
class MorphACWE(object):
    ...
    def step(self):
        ...
        outside_sum = np.finfo(np.float).eps if np.nan_to_num(outside_sum) == 0 else outside_sum
        inside_sum = np.finfo(np.float).eps if np.nan_to_num(inside_sum) == 0 else inside_sum

        c0 = data[outside].sum() / outside_sum
        c1 = data[inside].sum() / inside_sum
