...
def build_vgg_pirads(input_shape) -> Network:
    ...
    def modality_layer_out(L: list, location, modality_rules):
        ...
        L = connected(L, filters=2, l_scale=l_scale, activation_fn=tf.nn.sigmoid, name="OUT")
        _out = tf_layer(L)
        return {
            'out': _out,
    ...
    def network_output(pz_pirads, tz_pirads, location):
        _pz_pirads = tf_layer(pz_pirads)
        _tz_pirads = tf_layer(tz_pirads)
        weights = get_pz_tz_weights(location)
        prads = tf.where(tf.dtypes.cast(weights[:, 1], tf.bool), _tz_pirads, _pz_pirads)
        describe(prads)
        return {
            'out': prads,
            'out_soft': tf.nn.softmax(prads)
        }

    def pz_net(dwi, dce):
        #dwidce =  join([dropout(dwi, dropratio=0.125), dropout(dce, dropratio=0.125)])
        dwidce =  join([dropout(dwi, dropratio=0.0), dropout(dce, dropratio=0.0)])
        _out = connected(dwidce, filters=2, name="PZ_OUT", l_scale=l_scale, activation_fn=tf.nn.sigmoid)
        _out = tf_layer(_out)
        return {
            'out': _out,
            'out_soft': tf.nn.softmax(_out)
    ...
    def tz_net(dwi, t2):
        #t2dwi =  join([dropout(t2, dropratio=0.125), dropout(dwi, dropratio=0.125)])
        t2dwi =  join([dropout(t2, dropratio=0.0), dropout(dwi, dropratio=0.0)])
        _out = connected(t2dwi, filters=2, name="TZ_OUT", l_scale=l_scale, activation_fn=tf.nn.sigmoid)
        _out = tf_layer(_out)
        return {
            'out': _out,
            'out_soft': tf.nn.softmax(_out)
    ...
    def create_net(x) -> dict:
        with tf.variable_scope("NET"):
            ...
            with tf.variable_scope("PZ"):
                pz = pz_net(dwi_adc_n, dce_n)

            with tf.variable_scope("TZ"):
                tz = tz_net(dwi_adc_n, t2_n)

        net_out = network_output(pz, tz, x['location'])
        return {
            'NET': net_out,
            'DWI_ADC': dwi_adc_n,
            'DCE': dce_n,
    ...
    with tf.get_default_graph().as_default():
        ...
        with tf.device(Constants.device_type):
            ...
            loss_ratios = {
                'PZ_WCE_PZ': 50,  'TZ_WCE_TZ': 50, #Equivalent of 'NET_WCE" = 100
                'T2_WCE_TZ': 20,
                'T2_WCE_PZ': 5,
                'DCE_WCE_TZ': 5,
                'DCE_WCE_PZ': 20,
