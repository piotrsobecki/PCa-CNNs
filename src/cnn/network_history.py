...
class NetworkHistory:
    ...
    def __init__(self, network:Network, epochs: int):
        ...
        net_keys = ['DCE', 'DCE_PZ', 'DCE_TZ', 'DWI_ADC', 'DWI_ADC_TZ', 'DWI_ADC_PZ', 'T2', 'T2_PZ', 'T2_TZ', 'PZ', 'TZ',  'NET']
    ...
    def header(self):
        ...
        return "ITER\tBEST_SCORE\tTR\tVL\t" + t_p_k + '\t' + t_p_v# +'\t' + '\t'.join(train_cost_keys)+ '\t' + '\t'.join(val_cost_keys)
    ...
    def update(self, input:NetworkInputSplit, epoch, test: NetworkInput):
        ...
        if self.best_epoch < 0 or (input.no_val and epoch == self.epochs-1) or (not input.no_val and self.val_auc[self.best_epoch] <= self.val_auc[epoch]):
    ...
    def _feed_probabilities(self, ni: NetworkInput):
        probs_out = None
        for batch in _gen_batches(ni, Constants.batch_size, augment=False ):
            probs_batch = self.network.session.run(self.network.probabilities, feed_dict=batch.prepare_feed_dict(self.network))
    ...
    def row(self, epoch):
        train_aucs = '\t'.join(["%.3f" % self.train_auc_all[epoch][name] for name in self.auc_key_order if name in self.train_auc_all[epoch]])
        val_aucs = '\t'.join(["%.3f" % self.val_auc_all[epoch][name] for name in self.auc_key_order if name in self.val_auc_all[epoch]])
        ...
        return "%06d\t%.3f\t%.3f\t%.3f\t%s\t%s" % (epoch, bs, self.train_auc[epoch], self.val_auc[epoch], train_aucs, val_aucs)#, train_costs, val_costs)
